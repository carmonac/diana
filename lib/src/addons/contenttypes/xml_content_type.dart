...
class XmlContentType extends ContentType with Serializable, Deserializable {
  ...
  serialize(object) {
    ...
    if (object is List) {
      // Para listas, crear un elemento raíz que contenga todos los elementos
      ...
      for (final item in object) {
        if (item != null && DtoRegistry.isRegistered(item.runtimeType)) {
          ...
          if (serializedData != null) {
            buffer.write(_mapToXml('item', serializedData));
          }
        } else {
          // Para otros tipos, usar serialización recursiva
          buffer.write(_serializeItem('item', item));
        }
      }
    ...
    if (DtoRegistry.isRegistered(object.runtimeType)) {
      ...
      if (serializedData != null) {
        return _mapToXml('root', serializedData);
      }
    }
  ...
  String _mapToXml(String rootName, Map<String, dynamic> map) {
    ...
    // Agregar atributos (propiedades que empiecen con @)
